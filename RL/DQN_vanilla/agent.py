...
class Agent:
    ...
    def __init__(self, gamma, epsilon, lr, input_dims, batch_size,
                 steer_res=3, acc_res=3, max_mem_size=500000, epsilon_min=0.01, epsilon_decay=1e-5):
        """
    ...
    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            ...
            actions_steer, actions_acc = self.Q_eval.forward(state)
            action_steer = T.argmax(actions_steer)
            action_acc = T.argmax(actions_acc)

        else:
            action_steer = np.random.choice(self.action_space_steer)
            action_acc = np.random.choice(self.action_space_accelerate)

        action = np.array([action_steer, action_acc], dtype=np.int)
        return action
    ...
    def learn(self):
        ...
        q_eval_steer, q_eval_acc = self.Q_eval.forward(state_batch)
